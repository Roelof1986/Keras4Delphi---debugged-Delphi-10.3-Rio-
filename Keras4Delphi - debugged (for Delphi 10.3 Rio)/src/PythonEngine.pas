...
{$IFDEF MSWINDOWS}
  PYTHON_KNOWN_VERSIONS: array[1..10] of TPythonVersionProp =
    (
    (DllName: {'python33.dll'}'python3.dll'; RegVersion: '3.3'; APIVersion: 1013),
...
type
  ...
  public
    ...
    function   PyObjectAsDoubleArray( obj : PPyObject ) : Variant{TArray<Double>};
    function   PyObjectAsDBL( obj : PPyObject ) : {Variant}Double;
    ...
    function   GetSequenceItem_DBL(sequence: PPyObject; idx: Integer): {Variant}{varDouble}Double; // added
...
function TPythonEngine.GetSequenceItem_DBL(sequence: PPyObject;  // added.
  idx: Integer): {Variant}{varDouble}Double;
  var
    val : PPyObject;
  begin
    val := PySequence_GetItem( sequence, idx );
    try
      Result := {PyObjectAsVariant}PyObjectAsDBL( val );
    finally
      Py_XDecRef( val );
    end;
end;
...
function TPythonEngine.PyObjectAsDoubleArray( obj : PPyObject ) : {TArray<Double>}Variant;  // added
...
(*  if PyFloat_Check(obj) then
    ...
    begin
      ...
      if PyErr_Occurred = nil then
        begin
          // Convert a Python sequence into an array of Variant
          Result := VarArrayCreate( [0, seq_length-1], {varVariant}varDouble ); // this one works!
          for i := 0 to PySequence_Length( obj )-1 do
            Result[i] := GetSequenceItem_DBL( obj, i ); // --> Double kan niet naar Variant Array geconv worden. dus geen [i] mogelijk.
//          Result :=   GetSequenceItem_DBL( obj, i )
        end
      else // the object didn't implement the sequence API, so we return Null
        begin
          PyErr_Clear;
...
function TPythonEngine.PyObjectAsDBL( obj : PPyObject ) : {Variant}Double;
